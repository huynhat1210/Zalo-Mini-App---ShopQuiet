...
export const CommentsPage: React.FC = () => {
  ...
  const parseImages = (imgsJson?: string): string[] => {
    if (!imgsJson) return [];
    try {
      const parsed = JSON.parse(imgsJson);
      if (Array.isArray(parsed)) return parsed;
    } catch (e) {
      if (typeof imgsJson === 'string' && imgsJson.trim()) return [imgsJson.trim()];
    }
    return [];
  };
  ...
  return (
    <div className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-100 shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-1">
            Theo dõi, phản hồi và xử lý toàn bộ đánh giá sản phẩm từ người mua hàng Zalo Mini App
          </p>
        </div>
      ...
      {/* ── Comments List View ── */}
      {loading ? (
        <div className="bg-white p-12 rounded-3xl border border-slate-100 shadow-xs text-center space-y-3">
          <div className="w-8 h-8 border-4 border-[#0e6877] border-t-transparent rounded-full animate-spin mx-auto"></div>
          <p className="text-xs font-bold text-slate-500">Đang tải danh sách bình luận...</p>
        </div>
      ) : comments.length === 0 ? (
        <div className="bg-white p-12 rounded-3xl border border-slate-100 shadow-xs text-center space-y-3">
        ...
        <div className="space-y-4">
          {comments.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xs hover:shadow-md transition-all space-y-4"
              >
                {/* Comment Top Header */}
                ...
                {item.product && (
                  <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-100">
                    ...
                    <div className="flex-1 min-w-0">
                      <span className="text-[10px] font-extrabold text-[#0e6877] uppercase tracking-wider block">
                        Sản phẩm đánh giá #{item.product.id}
                      </span>
                    ...
                    {reviewImgs.map((img, idx) => {
                      ...
                      return (
                        <img
                          ...
                          className="w-20 h-20 rounded-2xl object-cover border border-slate-200 cursor-pointer hover:opacity-90 hover:scale-105 transition-all shadow-xs"
                        />
                      );
                    })}
                  </div>
