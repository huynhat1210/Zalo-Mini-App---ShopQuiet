...
export const VoucherWallet: React.FC<IVoucherWalletProps> = (props) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/45 backdrop-blur-xs flex items-center justify-center p-6 animate-fade-in">
      <div className="bg-white  text-slate-900  w-full max-w-sm rounded-3xl p-6 border border-[#f0edeb]  shadow-2xl space-y-4 animate-scale-up max-h-[75vh] flex flex-col text-left">
        <div className="flex items-center justify-between shrink-0 pb-2 border-b border-[#f5f3f0] ">
          ...
          <button
            onClick={onClose}
            className="p-1.5 bg-neutral-100  hover:bg-neutral-200 :bg-slate-700 rounded-full transition-colors border-none cursor-pointer flex items-center justify-center text-slate-500 "
          >
        ...
        <div className="overflow-y-auto flex-1 space-y-3 pr-1">
          {loading ? (
            ...
            userVouchers.map((v: any) => (
              <div
                key={v.code}
                className="border border-dashed border-[#0e6877]/30 /50 rounded-2xl overflow-hidden flex bg-[#fcfbfa]  shadow-2xs hover:shadow-xs transition-shadow"
              >
