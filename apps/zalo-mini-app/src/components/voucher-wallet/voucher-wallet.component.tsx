        ...
        <div className="overflow-y-auto flex-1 space-y-2.5 pr-1">
          {loading ? (
            ...
            userVouchers.map((v: any) => (
              <div
                key={v.code}
                className="border border-dashed border-teal-200 rounded-2xl overflow-hidden flex"
              >
                <div className="bg-gradient-to-br from-teal-500 to-teal-700 text-white px-3 py-3 flex flex-col items-center justify-center min-w-[72px] gap-0.5">
                  ...
                  <span className="text-[8px] font-bold opacity-80">
                    {v.type === "FREESHIP" ? "Freeship" : "Giảm"}
                  </span>
                </div>
                <div className="flex-1 px-3 py-2.5 flex flex-col justify-center gap-0.5 text-left">
                  <div className="flex items-center justify-between">
                    <span className="font-black text-xs text-textColor tracking-widest">
                      {v.code}
                    </span>
                    <button
                      onClick={() => {
                  ...
                  <p className="text-[10px] text-textColor-variant">
                    {v.type === "PERCENT"
                      ...
                      : v.type === "FIXED"
                        ...
                        : "Miễn phí vận chuyển"}
                    {v.minOrderVal > 0
                      ? ` • Đơn tối thiểu ${v.minOrderVal.toLocaleString("vi-VN")}đ`
                      : ""}
                  </p>
                  {v.expiresAt && (
