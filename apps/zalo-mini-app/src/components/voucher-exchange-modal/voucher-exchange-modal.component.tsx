...
export const VoucherExchangeModal: React.FC<IVoucherExchangeModalProps> = (props) => {
  const { isOpen, onClose, gamificationData, exchangeVoucher, onExchangeSuccess } = props;
    ...
    <div className="fixed inset-0 z-[100] bg-black/45 backdrop-blur-xs flex items-center justify-center p-6 animate-fade-in">
      <div className="bg-white w-full max-w-sm rounded-3xl p-6 border border-[#f0edeb] shadow-2xl space-y-4 animate-scale-up max-h-[70vh] flex flex-col">
        <div className="flex items-center justify-between shrink-0">
          <h3 className="text-xs font-bold text-textColor uppercase tracking-wider">🎁 Chợ đổi Voucher</h3>
          <button
            onClick={onClose}
        ...
        <p className="text-[10px] text-textColor-variant font-semibold">
          Điểm tích lũy của bạn: <span className="text-primary font-bold">{gamificationData?.points || 0}</span>
        </p>
