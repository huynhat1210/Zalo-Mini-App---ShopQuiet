...
export const ChatOverlay: React.FC<ChatOverlayProps> = ({ onClose }) => {
  ...
  // 3. Check if product context has already been sent (to avoid re-showing banner)
  const alreadySentContext = chatContextProduct
    ? messages.some(
        (m) =>
          m.sender === "USER" &&
          m.content.startsWith("[PRODUCT_CONTEXT]") &&
          m.content.includes(`"id":${chatContextProduct.id}`),
      )
    : false;

  const handleSendProductContext = () => {
    if (!chatContextProduct) return;
    handleSendMessage(
  ...
  const renderMessageContent = (content: string) => {
    if (content.startsWith("[PRODUCT_CONTEXT]")) {
      try {
        ...
        const fullImg = imgUrl.startsWith("http")
          ? imgUrl
          : `${serverUrl}${imgUrl}`;
        ...
        return (
          ...
          >
            <img
              src={fullImg}
              alt={prod.name}
              className="w-12 h-12 object-cover rounded-xl border border-teal-200 shrink-0"
            />
            <div className="flex-1 min-w-0">
              <span className="text-[8px] bg-teal-600 text-white px-1.5 py-0.5 rounded font-black uppercase tracking-wider">
                Đang xem
              </span>
              <h4 className="text-xs font-bold text-teal-950 truncate mt-1 leading-snug">
                {prod.name}
              </h4>
        ...
        <div className="bg-white px-6 py-4 flex items-center justify-between border-b border-[#f0edeb] shadow-xs">
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <p className="text-[9px] text-textColor-variant font-medium mt-0.5">
                Thường phản hồi trong vài phút
              </p>
            </div>
          </div>
          <span className="text-[8px] font-extrabold text-[#526069]/65 bg-neutral-100 px-2.5 py-1 rounded-full uppercase tracking-wider border border-[#f0edeb]">
            Real-time
          </span>
        </div>

        {/* Product Context Confirmation Banner */}
        {chatContextProduct && !alreadySentContext && !loading && (
          <div className="mx-4 mt-3 bg-teal-50 border border-teal-200 rounded-2xl p-3 flex items-center gap-3 shadow-xs animate-fade-in">
            <div className="flex-1 min-w-0">
              <p className="text-[9px] font-extrabold uppercase tracking-wider text-teal-700 mb-1">
                📦 Gửi thông tin sản phẩm?
              </p>
              <p className="text-[11px] font-bold text-teal-950 truncate">
                {chatContextProduct.name}
              </p>
              <p className="text-[10px] text-teal-700 font-semibold mt-0.5">
                {chatContextProduct.price?.toLocaleString("vi-VN")} đ
              </p>
            </div>
            <div className="flex flex-col gap-1.5 shrink-0">
              <button
                onClick={handleSendProductContext}
                className="px-3 py-1.5 bg-teal-600 hover:bg-teal-700 text-white text-[10px] font-black rounded-full active:scale-95 transition-all border-none cursor-pointer uppercase tracking-wider"
              >
                Gửi
              </button>
              <button
                onClick={handleDismissProductContext}
                className="px-3 py-1.5 bg-white hover:bg-neutral-100 text-neutral-500 text-[10px] font-bold rounded-full active:scale-95 transition-all border border-neutral-200 cursor-pointer uppercase tracking-wider"
              >
                Bỏ qua
              </button>
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4.5 scrollbar-thin">
          {loading ? (
            ...
            <div className="h-full flex flex-col items-center justify-center p-8 text-center text-neutral-400 space-y-3">
              <div className="w-14 h-14 bg-teal-50 text-teal-600 rounded-full flex items-center justify-center text-2xl mx-auto shadow-xs">
                💬
              </div>
              <div>
                <p className="text-xs font-bold text-textColor">
                  Trò chuyện với ShopQuiet
                </p>
                <p className="text-[10px] text-textColor-variant mt-1 leading-relaxed max-w-[200px] mx-auto">
                  Hãy gửi tin nhắn để được nhân viên hỗ trợ trực tiếp nhanh
                  nhất.
                </p>
              </div>
            </div>
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <span
                      className={`block text-[7px] text-right mt-1.5 font-bold uppercase tracking-wider ${
