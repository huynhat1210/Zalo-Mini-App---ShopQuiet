    ...
    (set, get) => ({
      ...
      updateItemVariant: (productId, oldSize, newSize, oldColor, newColor) => {
        ...
        if (existingNewVariant) {
          ...
          nextCart = [
            ...nextCart,
            { ...oldItem, size: newSize, color: newColor },
          ];
        }

        set({ cart: nextCart });
        apiRequest<any[]>("/cart/variant", "PUT", {
          productId,
          oldSize,
          newSize,
          oldColor,
          ...
          .catch((error) => {
            ...
            get().showToast(
              "Không thể đổi phân loại. Vui lòng thử lại.",
              "warning",
            );
          });
      },
      toggleSavedItem: (product) => {
        ...
        const next = exists
          ? get().savedItems.filter((item) => item.id !== product.id)
          : [...get().savedItems, product];
        set({ savedItems: next });
        ...
        if (exists) {
          ...
          apiRequest("/favorites", "POST", { productId: product.id }).catch(
            console.error,
          );
        }
      },
      isSavedItem: (productId) =>
        get().savedItems.some((item) => item.id === productId),
      ...
      claimDailyReward: async () => {
        ...
        try {
          const res = await apiRequest<any>(
            "/gamification/claim-daily-reward",
            "POST",
          );
          if (res && res.success) {
            get().showToast(
              res.message || "Điểm danh hàng ngày thành công!",
              "success",
            );
            await get().fetchGamificationData();
          } else {
            get().showToast(
              res?.message || "Hôm nay bạn đã điểm danh rồi.",
              "warning",
            );
          }
        } catch (e: any) {
          get().showToast(
            e?.message || "Hôm nay bạn đã điểm danh rồi!",
            "warning",
          );
        }
      },
        ...
        try {
          ...
          if (user && user.id) {
            data = await apiRequest<any>(
              `/recommendations/personalized?zaloUserId=${user.id}&limit=8`,
              "GET",
            );
          }
          ...
          set({
            recommendations: Array.isArray(data) ? data : data?.data || [],
          });
        } catch (e) {
      ...
      exchangeVoucher: async (voucherCode: string, pointsCost: number) => {
        ...
        try {
          ...
          if (res && res.success) {
            get().showToast(
              res.message || "Đổi voucher thành công!",
              "success",
            );
            await get().fetchGamificationData();
            return true;
            ...
            get().showToast(
              res?.message || "Không đủ điểm để đổi voucher này!",
              "warning",
            );
            return false;
          }
