    ...
    <PageCast className="bg-surface flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5.5 space-y-5 text-xs text-left pb-28">
        ...
        <div className="bg-white rounded-2xl border border-[#f0edeb] p-4.5 space-y-3.5 shadow-xs relative text-left">
          ...
          {!isPendingPayment && selectedOrder.status !== 'CANCELLED' && (
            <div className="pt-1.5 space-y-3">
              ...
              <div className="flex flex-col gap-3 pl-2.5 relative border-l-2 border-neutral-100 mt-1.5">
                {[
                  ...
                  { label: 'Đang chuẩn bị', desc: 'Kho hàng đang đóng gói sản phẩm của bạn', active: selectedOrder.status === 'PROCESSING' || selectedOrder.status === 'SHIPPED' || selectedOrder.status === 'DELIVERED' },
                  { label: 'Đang giao hàng', desc: 'Đơn hàng đã bàn giao cho đơn vị vận chuyển', active: selectedOrder.status === 'SHIPPED' || selectedOrder.status === 'DELIVERED' },
                  { label: 'Đã hoàn thành', desc: 'Đơn hàng đã được giao nhận thành công', active: selectedOrder.status === 'DELIVERED' }
                ].map((step, sIdx) => (
          ...
          {selectedOrder.status === 'CANCELLED' && (
            <div className="pt-2 flex items-center gap-2 text-red-600 text-[10px] font-bold">
              <span>✕</span>
