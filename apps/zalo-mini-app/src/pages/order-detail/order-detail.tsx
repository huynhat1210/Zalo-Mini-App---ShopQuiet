...
export const OrderDetail: React.FC<IOrderDetailProps> = (_props) => {
  ...
  const isCancellable = ["PROCESSING", "PENDING_PAYMENT"].includes(
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5.5 space-y-5 text-xs text-left pb-28">
        ...
        <div className="bg-white rounded-2xl border border-[#f0edeb] p-4.5 space-y-3.5 shadow-xs relative text-left">
          <div className="flex justify-between items-center pb-2.5 border-b border-[#f0edeb]">
            ...
            <span
              className={`text-[9px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full ${statusClass}`}
            >
              {statusLabel}
            </span>
          </div>
                ...
                {(selectedOrder.status === "CANCELLED"
                  ...
                  <div key={sIdx} className="relative pl-5 text-xs">
                    ...
                    <div className="space-y-0.5">
                      <p
                        className={`font-bold ${step.active ? (step.isCancelled ? "text-red-600" : "text-textColor") : "text-textColor-variant/50"}`}
                      >
                        {step.label}
                      </p>
                      <p
                        className={`text-[10px] ${step.active ? (step.isCancelled ? "text-red-500/80" : "text-textColor-variant") : "text-textColor-variant/40"}`}
                      >
                        {step.desc}
                      </p>
                    </div>
                ...
                <div className="text-right">
                  ...
                  {["PROCESSING", "SHIPPED"].includes(selectedOrder.status) && (
                    <p className="text-[10px] text-textColor-variant mt-0.5">
                      {getDeliveryStatusText(
                        getRemainingDays(selectedOrder.estimatedDeliveryDate),
                      )}
                    </p>
                  )}
                </div>
            ...
            {selectedOrder.items?.map((item: IOrderItem, idx: number) => (
              <div key={idx} className="flex justify-between items-center p-4">
                <div className="text-left space-y-1">
                  <span className="font-semibold text-textColor leading-tight block">
                    {item.product?.name}
                  </span>
                        ...
                        {parsed.map((img: string, idx: number) => {
                          ...
                          return (
                            <img
                              ...
                              src={full}
                              alt="Bằng chứng"
                              className="w-12 h-12 object-cover rounded-xl border border-indigo-150"
                            />
                          );
                        })}
                      </div>
        ...
        <div className="fixed inset-0 z-[100] bg-black/45 backdrop-blur-xs flex items-center justify-center p-6 text-left">
          <div className="bg-white w-full max-w-sm rounded-3xl p-6 border border-[#f0edeb] shadow-2xl space-y-4 animate-scale-up">
            <div className="flex justify-between items-center pb-2 border-b border-neutral-100">
              <h3 className="text-xs font-black text-textColor uppercase tracking-wider">
            ...
            <div className="space-y-1.5">
              <label className="text-[9px] font-extrabold text-[#526069]/70 uppercase tracking-widest block">
                Lý do hoàn trả
              </label>
              <select
                value={returnReason}
              ...
              >
                <option value="Sản phẩm lỗi kỹ thuật / không hoạt động">
                  Sản phẩm lỗi kỹ thuật / không hoạt động
                </option>
                <option value="Sản phẩm hư hỏng do vận chuyển">
                  Sản phẩm hư hỏng do vận chuyển
                </option>
                <option value="Gửi sai sản phẩm / thiếu hàng">
                  Gửi sai sản phẩm / thiếu hàng
                </option>
                <option value="Hàng giả / hàng nhái">
                  Hàng giả / hàng nhái
                </option>
            ...
            <div className="space-y-2">
              <label className="text-[9px] font-extrabold text-[#526069]/70 uppercase tracking-widest block">
                Ảnh bằng chứng lỗi (Tối đa 3 ảnh)
              </label>
              <div className="flex flex-wrap gap-2.5">
                {returnImages.map((url, idx) => (
                  <div
                    key={idx}
                    className="w-14 h-14 rounded-xl border border-neutral-200 overflow-hidden bg-neutral-50 relative"
                  >
                    <img
                      src={
                        url.startsWith("http")
                  ...
                  <label className="w-14 h-14 rounded-xl border-2 border-dashed border-neutral-300 hover:border-primary flex flex-col items-center justify-center cursor-pointer bg-neutral-50 transition-colors">
                    ...
                    {uploadingReturnImg ? (
                      ...
                      <>
                        ...
                        <span className="text-[7.5px] text-neutral-450 font-bold uppercase mt-0.5">
                          Ảnh
                        </span>
                      </>
                    )}
