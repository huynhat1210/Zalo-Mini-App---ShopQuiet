...
export const PaymentSimulate: React.FC<IPaymentSimulateProps> = (_props) => {
  ...
  const handlePaySuccess = async () => {
    try {
      ...
      const userObj = localStorage.getItem("zalo_profile_custom")
        ? JSON.parse(localStorage.getItem("zalo_profile_custom")!)
        : null;
      const userId = userObj
        ? userObj.zaloId || userObj.id || "cust-zalo-id-1"
        : "cust-zalo-id-1";
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          itemsCount: (selectedOrder.items ?? []).reduce(
              ...
              {(selectedOrder.items ?? []).map(
                (item: IOrderItem, idx: number) => (
                  <div key={idx} className="py-2.5 flex justify-between">
                    ...
                    <span className="font-semibold text-textColor/75">
                      x{item.quantity}
                    </span>
                  </div>
                ),
