...
export const Notifications: React.FC<INotificationsProps> = (_props) => {
  ...
  const [activeCategory, setActiveCategory] = useState<'order' | 'promo' | 'system'>('order');
    ...
    return (
      <PageCast className="bg-[#f7f7f7] relative flex flex-col w-full h-full overscroll-none scrollbar-none items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-[10px] font-bold text-textColor-variant tracking-wider uppercase">Đang tải thông báo...</span>
        </div>
      </PageCast>
    );
  }
  ...
  const getOrderId = (title: string, content: string) => {
    ...
    // Match #123 format
    const matchHashNum = titleText.match(/#(\d+)/) || contentText.match(/#(\d+)/);
    if (matchHashNum) return matchHashNum[1];

    // Match "đơn hàng 123" or "đơn hàng số 123" format
    const matchOrderNum = titleText.match(/(?:đơn hàng|Mã đơn|mã đơn|Đơn hàng)\s+(?:số\s+)?(\d+)/i) || 
                          contentText.match(/(?:đơn hàng|Mã đơn|mã đơn|Đơn hàng)\s+(?:số\s+)?(\d+)/i);
  ...
  const filteredNotifications = notifications.filter((item) => {
    ...
    if (activeCategory === 'order') {
      ...
      return itemType === 'promo' || itemType === 'voucher' || itemType === 'discount';
    } else {
      return itemType !== 'order' && itemType !== 'promo' && itemType !== 'voucher' && itemType !== 'discount';
    }
  });
  ...
  return (
    <PageCast className="bg-surface relative flex flex-col w-full h-full overscroll-none scrollbar-none animate-fade-in">
      ...
      <div className="bg-white/95 backdrop-blur-md px-6 py-4 flex items-center justify-between border-b border-[#f0edeb] sticky top-0 z-30 shadow-xs">
        <button onClick={() => setActiveTab('home')} className="p-1.5 -ml-1.5 hover:bg-[#f0edeb] rounded-full transition-colors active:scale-95 border-none bg-transparent cursor-pointer">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4 pb-28 text-left animate-fade-in">
        {filteredNotifications.length === 0 ? (
          <EmptyStateComponent
            title={
              activeCategory === 'order' ? 'Không có thông báo đơn hàng' : 
              activeCategory === 'promo' ? 'Không có khuyến mãi mới' : 
              'Không có thông báo hệ thống'
            }
            description={
              activeCategory === 'order' ? 'Bạn chưa nhận được thông báo nào về đơn hàng.' :
              activeCategory === 'promo' ? 'Voucher và tin tức khuyến mãi sẽ hiển thị tại đây.' :
          ...
          <div className="space-y-3.5">
            {filteredNotifications.map((item) => {
              ...
              const discountCode = activeCategory === 'promo' ? getDiscountCode(item.content) : undefined;
              return (
                <div 
                  key={item.id} 
                  onClick={() => {
                ...
                >
                  ...
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                    activeCategory === 'promo' ? 'bg-amber-50 text-amber-700' : 'bg-primary-light text-primary'
                  }`}>
                    {activeCategory === 'promo' ? (
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    <p className="text-[11px] text-textColor-variant leading-relaxed font-medium">{item.content}</p>
